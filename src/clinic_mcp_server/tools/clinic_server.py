...
from clinic_mcp_server.domain.errors import ClinicError, ConflictError, NotFoundError, ValidationError
...
@mcp.tool()
def search_doctors(
    specialty: Optional[str] = None, min_rank: Optional[float] = None, max_fee: Optional[float] = None
...
def search_available_appointments(
    specialty: str, doctor_name: Optional[str] = None, start_date: Optional[str] = None, end_date: Optional[str] = None
...
@mcp.tool()
def get_appointment_slot(slot_id: int) -> Optional[AppointmentSlot]:
    svc = get_service()
    try:
